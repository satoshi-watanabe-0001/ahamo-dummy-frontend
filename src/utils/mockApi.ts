...
%%EOF`;
  ...
  getContractDetails: async (id: string) => {
    ...
    if (contract) {
      ...
      const options = contract.options?.map((optId: string) => mockOptions.find(o => o.id === optId)).filter(Boolean) || [];
      const contractDetails = {
        ...contract,
        customerName: contract.customerName || 'テストユーザー',
        plan: plan || plans[0],
        optionDetails: options,
        totalMonthlyFee: (plan?.monthlyFee || 0) + options.reduce((sum: number, opt: any) => sum + (opt?.monthlyFee || 0), 0),
        contractPeriod: {
          startDate: contract.createdAt,
          endDate: null
        }
      };
      return createMockResponse(contractDetails);
    }
    throw createMockError('契約が見つかりません', 404);
  },
  getContractHistory: async (_id: string) => {
    await delay(200 + Math.random() * 300);
    const history = [
      {
        id: 1,
  ...
  getCurrentOptions: async (id: string) => {
    await delay(200 + Math.random() * 300);
    const contracts = getStoredDataTyped(STORAGE_KEYS.CONTRACTS, mockContracts);
    const contract = contracts.find(c => c.id === id);
    if (contract) {
  ...
  changePlan: async (id: string, data: any) => {
    await delay(500 + Math.random() * 700);
    const contracts = getStoredDataTyped(STORAGE_KEYS.CONTRACTS, mockContracts);
    const contract = contracts.find(c => c.id === id);
    if (contract) {
      contract.planId = data.newPlanId;
      contract.updatedAt = new Date().toISOString();
      setStoredData(STORAGE_KEYS.CONTRACTS, contracts);
      return createMockResponse(contract);
    }
    throw createMockError('契約が見つかりません', 404);
  },
  simulatePlanChange: async (_id: string, data: any) => {
    await delay(300 + Math.random() * 500);
    const plans = getStoredDataTyped(STORAGE_KEYS.PLANS, mockPlans);
    const newPlan = plans.find(p => p.id === data.newPlanId);
    if (newPlan) {
      const simulation = {
        baseFee: newPlan.monthlyFee,
        callFee: 0,
        dataFee: 0,
        optionFee: 500,
        discount: 0,
        taxExcluded: newPlan.monthlyFee + 500,
        taxIncluded: Math.round((newPlan.monthlyFee + 500) * 1.1)
      };
      return createMockResponse(simulation);
    }
    throw createMockError('プランが見つかりません', 404);
  },
  getAvailablePlans: async (_id: string) => {
    await delay(200 + Math.random() * 300);
    const plans = getStoredDataTyped(STORAGE_KEYS.PLANS, mockPlans);
    return createMockResponse(plans);
  },
  addOption: async (id: string, data: any) => {
    ...
    if (contract) {
      ...
      if (!contract.options.includes(data.optionId)) {
        contract.options.push(data.optionId);
      }
      contract.updatedAt = new Date().toISOString();
      setStoredData(STORAGE_KEYS.CONTRACTS, contracts);
      return createMockResponse(contract);
  ...
  removeOption: async (id: string, optionId: string) => {
    ...
    if (contract && contract.options) {
      contract.options = contract.options.filter((opt: string) => opt !== optionId);
      contract.updatedAt = new Date().toISOString();
      setStoredData(STORAGE_KEYS.CONTRACTS, contracts);
      return createMockResponse(contract);
    }
  ...
  suspendOption: async (id: string, _optionId: string) => {
    await delay(400 + Math.random() * 600);
    const contracts = getStoredDataTyped(STORAGE_KEYS.CONTRACTS, mockContracts);
    const contract = contracts.find(c => c.id === id);
    if (contract) {
  ...
  getAvailableOptions: async (_id: string) => {
    await delay(200 + Math.random() * 300);
    return createMockResponse(mockOptions);
  },
  ...
  changePlan: (_id: string, _data: any) => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          data: {
  ...
  simulatePlanChange: (_id: string, _data: any) => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          data: {
  ...
  addOption: (_id: string, _data: any) => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          data: {
  ...
  removeOption: (_id: string, _optionId: string) => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          data: {
  ...
  suspendOption: (_id: string, _optionId: string) => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          data: {
